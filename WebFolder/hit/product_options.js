...
function fetchsuboptions() {
	$.getJSON("loadproductoptions.cgi?start=0&end=100&tablname=product_options&option_uuid="+$('#uuid').val(),function(result){
		var htmlStr="";
		if(result.values){
  	...
  	bind();
			...
			$("#ProductForm").validate({
				...
				submitHandler: function (form) {
    				...
    				var sub_options= $('#sub_options').val(); 
    				alert(sub_options);
					$.ajax({
						type: "POST",
						url: "saveproductoptions.cgi",
						data: {'uuid' : uuid, "sub_options" : sub_options,"name" :name, "status": status},
						cache: false,
