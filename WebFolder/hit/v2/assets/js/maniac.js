	...
	loadvalidator: function () {
		$('#signInform').bootstrapValidator({
			...
			/**submitHandler: function(validator, form, submitButton) {
				var jsonRow = 'set_session.cgi?platform='+call4DurlBool;
		...
		$('#TimeslipForm').bootstrapValidator({
			message: 'This value is not valid',
			excluded: ':disabled',
			fields: {
				Task_ID: {
			...
			submitHandler: function(validator, form, submitButton) {
				...
				if($('#lunchinterval').is(":checked")){
					if($('#Timeslip_uuid').val()!=""){
						//alert($('#interval_time').val());
						if($('#interval_time').val()==""){
							if ($("#setBreakStartTime").val()==""){ 
								__alertMessage('Please select interval start time'); 
								return false;
								submitFormBool=false;
							}
							if ($("#setBreakEndTime").val()==""){ 
					...
					inv_no: {
						validators: {
							...
							greaterThan: {
								value: 1,
								inclusive: false,
								message: 'The invoice number to be greater than 0'
							}
						}
					},
					date_received: {
					...
					amount: {
						validators: {
							...
							greaterThan: {
								value: 1,
								inclusive: false,
								message: 'The amount to be greater than 0'
							}
						}
					}
				}
